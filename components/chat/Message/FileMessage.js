...
const FileMessage = ({
  ...
  useEffect(() => {
    if (msg?.file) {
      ...
      console.debug("Preview URL generated:", {
        filename: JSON.parse(msg.file).filename,
        url,
      });
    }
  }, [msg?.file]);
  ...
  const handleFileDownload = async (e) => {
    ...
    try {
      ...
      const baseUrl = files.url;

      const link = document.createElement("a");
      link.href = baseUrl;
      link.download = getDecodedFilename(files.originalname);
  ...
  const renderFilePreview = () => {
    ...
    const originalname = getDecodedFilename(
      files.originalname || "Unknown File"
    );
    const size = fileService.formatFileSize(files.size || 0);
